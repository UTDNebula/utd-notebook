...
export default function ReportCard({ reportedFile }: ReportCardProps) {
  ...
  return (
    <BaseCard variant="interactive" className = "flew grow flex-col">
        ...
        <div className={`rounded-full w-fit px-4 py-1.5 m-3 ${statusStyles[status]} text-xs`}>
          {statusLabels[status]}
        </div>

        {/* PDF THUMBNAIL */}
        <Link href={`/notes/${reportedFile.id}`}>
          <PdfThumbnail files={[{ file: reportedFile.publicUrl, name: reportedFile.name }]} className="aspect-2/1"/>
        </Link>
        {/* NOTES HEADER: title, uploader, and reporter */}
        <div className="m-3">
          <div className="flex flex-col gap-2 mb-4">
            <Link href={`/notes/${reportedFile.id}`}>
              <h3 className="line-clamp-1 text-xl font-semibold" title={reportedFile.name}>
                {reportedFile.name}
              </h3>
            </Link>
          ...
          <div>
            <div className="flex flex-cols items-center">
                <h4 className="text-lg whitespace-nowrap mr-2">Report Details: </h4>
